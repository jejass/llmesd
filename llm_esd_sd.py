...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', help='Path to save model', type=str, default='esd-models/llmsd/')
    parser.add_argument('--device', help='cuda device to train on', type=str, required=False, default='cuda:0')

    parser.add_argument('--num_prompts', type=int, default=10, help='how many prompts to generate via LLM')

    
    parser.add_argument('--fewshot', dest='fewshot', action='store_true', help='enable few-shot examples in LLM prompt')
    parser.set_defaults(fewshot=False)
    parser.add_argument('--cot', dest='cot', action='store_true', help='enable chain-of-thought hint in LLM prompt')
    parser.set_defaults(cot=False)
    parser.add_argument('--context_info', dest='context_info', action='store_true', help='enable extra context in LLM prompt')
    parser.set_defaults(context_info=False)
    ...
    def bool_tag(prefix, v): return f"{prefix}{1 if v else 0}"
    tag = "-".join([
        bool_tag("fs", use_fewshot),
        bool_tag("cot", use_cot),
        bool_tag("ctx", use_ctx),
        f"n{num_prompts}"
    ])

    # 安全的名称片段
    safe_concept = erase_concept.replace(' ', '_')
    safe_from = (erase_concept_from if erase_concept_from else erase_concept).replace(' ', '_')


    os.makedirs("prompts", exist_ok=True)
    prompt_out = f"prompts/{safe_concept}-{tag}.json"

    erase_prompt_list_dict = generate_concept_prompts(concept=args.erase_concept,
        num_prompts=num_prompts,
        add_target_instruction=True,
        add_output_constraint=True,
        add_role_spec=True,
        add_fewshot=use_fewshot,
        add_cot=use_cot,
        add_context_info=use_ctx,
        output_file=prompt_out
    )
   
    if isinstance(erase_prompt_list_dict, dict) and 'result' in erase_prompt_list_dict:
        erase_prompt_list = erase_prompt_list_dict['result']
    else:
        raise ValueError("generate_concept_prompts() did not return expected {'result': [...]} structure.")

    with torch.no_grad():
        erase_embeds_list = []
        for prompt in erase_prompt_list:
            ...
            erase_embeds_list.append(embeds.to(device))
    ...
    for iteration in pbar:
        ...
        with torch.no_grad():
            ...
            if erase_concept_from is not None:
                noise_pred_erase_from_global = pipe.unet(
                        xt,
                        run_till_timestep_scheduler,
                ...
                noise_pred_erase_from_global = None
    

        losses_per_prompt = []
        for erase_embeds in erase_embeds_list:
            with torch.no_grad():
                pipe.unet = base_unet
                ...
                noise_pred_erase_from = (noise_pred_erase_from_global if noise_pred_erase_from_global is not None else noise_pred_erase)
        ...
        loss = sum(losses_per_prompt) / len(losses_per_prompt)
        loss.backward()
    ...
    save_file(esd_param_dict, f"{save_path}/esd-{erase_concept.replace(' ', '_')}-from-{erase_concept_from.replace(' ', '_')}-{train_method.replace('-','')}-{tag}.safetensors")
